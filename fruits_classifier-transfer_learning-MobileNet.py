# fruit_classifier-transfer_learning-MobileNet.py
#
...
from tensorflow.keras.applications import MobileNet 					            # load MobileNet library
...
new_MobileNet_model.compile(loss='categorical_crossentropy', optimizer=optimizers.SGD(lr, momentum), metrics=['accuracy'])


# Train the model
print("[INFO] Train the model on training data")

history = new_MobileNet_model.fit(
			train_generator,
			validation_data=validation_generator,
			epochs=epochs, 
			verbose=1)

#new_MobileNet_model.save('MobileNet_TransferLearning_Fruits360.h5')
...
#pyplot.savefig('fruits_classifier_MobileNet-training_acc.png')
pyplot.show()
...
#pyplot.savefig('fruits_classifier_MobileNet-training_loss.png')
pyplot.show()
...
results = new_MobileNet_model.evaluate(x_test, y_test, batch_size=batch_size, verbose=2)

print('Testing Loss, Testing Acc:', results)

pred = new_MobileNet_model.predict(x_test)
...
#pyplot.savefig('fruits_classifier_MobileNet-confusion-matrix.png')
pyplot.show()
