# fruit_classifier-transfer_learning-InceptionV3.py
#
...
from tensorflow.keras.applications.inception_v3 import InceptionV3 					# load InceptionV3 library
...
# generate training data using ImageDataGenerator
...
# Import the pre-trained model InceptionV3 and use the weights from ImageNet
...
modified_Inception_output = base_model.output
modified_Inception_output = GlobalAveragePooling2D()(modified_Inception_output)
modified_Inception_output = Dense(512, activation='relu')(modified_Inception_output)
modified_Inception_output = Dropout(0.5)(modified_Inception_output)
modified_Inception_output = Dense(num_classes, activation='softmax')(modified_Inception_output)

# Create the new model by using Input and Output layers
new_Inception_model = Model(inputs=base_model.input, outputs=modified_Inception_output)

new_Inception_model.summary()


# Compile the model with categorical crossentropy loss function and SGD optimizer
new_Inception_model.compile(loss='categorical_crossentropy', optimizer=optimizers.SGD(lr, momentum), metrics=['accuracy'])


# Train the model
print("[INFO] Train the model on training data")

history= new_Inception_model.fit(
			train_generator, 
			validation_data=validation_generator, 
			epochs=epochs, 
			verbose=1)

#new_Inception_model.save('InceptionV3_TransferLearning_Fruits360.h5')
...
#pyplot.savefig('fruits_classifier_InceptionV3-training_acc.png')
pyplot.show()
...
#pyplot.savefig('fruits_classifier_InceptionV3-training_loss.png')
pyplot.show()
...
results = new_Inception_model.evaluate(x_test, y_test, batch_size=batch_size, verbose=2)

print('Testing Loss, Testing Acc:', results)

pred = new_Inception_model.predict(x_test)
...
#pyplot.savefig('fruits_classifier_InceptionV3-confusion-matrix.png')
pyplot.show()