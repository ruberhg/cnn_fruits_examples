# fruit_classifier-transfer_learning-ResNet50.py
#
...
from tensorflow.keras.applications import ResNet50 					                # load ResNet50 library
...
# Import the pre-trained model ResNet50 and use the weights from ImageNet
base_model = ResNet50(
...
modified_ResNet50_output = base_model.output
...
modified_ResNet50_output = Dense(num_classes, activation='softmax')(modified_ResNet50_output)

# Create the new model by using Input and Output layers
new_ResNet50_model = Model(inputs=base_model.input, outputs=modified_ResNet50_output)

new_ResNet50_model.summary()


# Compile the model with categorical crossentropy loss function and SGD optimizer
new_ResNet50_model.compile(loss='categorical_crossentropy', optimizer=optimizers.SGD(lr, momentum), metrics=['accuracy'])


# Train the model
print("[INFO] Train the model on training data")

#history= new_ResNet50_model.fit(x=x_train, y=y_train, batch_size=batch_size, epochs=epochs, verbose=2, validation_split=0.1)

history = new_ResNet50_model.fit(train_generator, validation_data=validation_generator, epochs=epochs, verbose=1)

#new_ResNet50_model.save('ResNet50_TransferLearning_Fruits360.h5')
...
pyplot.savefig('fruits_classifier_ResNet50-training_acc-1.png')
#pyplot.show()
...
pyplot.savefig('fruits_classifier_ResNet50-training_loss-1.png')
#pyplot.show()
...
results = new_ResNet50_model.evaluate(x_test, y_test, batch_size=batch_size, verbose=2)

print('Testing Loss, Testing Acc:', results)

pred = new_ResNet50_model.predict(x_test)
...
pyplot.savefig('fruits_classifier_ResNet50-confusion-matrix-1.png')
#pyplot.show()
